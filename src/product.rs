...
pub async fn fetch_product(
    ...
    for row in rows {
        let product = format!("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}",
                              row.get::<&str, i64>("序号"), SPLITER,
                              row.get::<&str, &str>("node_name"), SPLITER,
                              row.get::<&str, &str>("物料号"), SPLITER,
                              row.get::<&str, &str>("规格型号"), SPLITER,
                              row.get::<&str, &str>("状态"), SPLITER,
                              row.get::<&str, &str>("执行标准"), SPLITER,
                              row.get::<&str, &str>("生产厂家"), SPLITER,
                              row.get::<&str, &str>("炉号"), SPLITER,
                              row.get::<&str, i32>("库存长度"), SPLITER,
                              row.get::<&str, f64>("库存重量"), SPLITER,
                              row.get::<&str, &str>("备注"), SPLITER
        );
    ...
    if user.name != "" {
        ...
        let sql = if da.len() == 1 {
            ...
            format!("select pi.单号id, max(类别) 类别, max(日期) 日期, sum(pi.数量)::int 数量,
                        ...
                        group by pi.单号id
                    union all
                    (select di.单号id, max(类别) 类别, max(日期) 日期, sum(数量)::int 数量,
                        sum(长度*数量)::int as 总长, sum(理重) 重量, max(di.备注) 备注
                        from document_items di
                        join documents on 单号 = di.单号id
                        where di.单号id like 'XS%' and 物料号 = '{}' and 出库完成 = false  {}
                        group by di.单号id
                        order by di.单号id desc)", da[0], NOT_DEL_SQL, da[0], NOT_DEL_SQL)
        };
