...
function get_date(cate) {
    let n1 = 1, n2 = 7, type = "month";
    if (cate == "按年") {
        n1 = 10;
        n2 = 4;
        type = "year";
    ...
    let value = `${date1.substring(0, n2)} - ${date2.substring(0, n2)}`
...
function set_cate_date(cate) {
    let date_s = get_date(cate).split(" - ");
...
document.querySelector('#statis-cate').addEventListener('change', function () {
    if (this.value == "按月") {
        ...
        document.querySelector('#search-date').value = "24";
    }
});
